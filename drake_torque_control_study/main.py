...
from drake_torque_control_study import debug
import drake_torque_control_study.controllers as m
from drake_torque_control_study.controllers import (
    DiffIkAndId,
    Gains,
    Osc,
    OscGains,
...
from drake_torque_control_study.multibody_extras import (
    get_frame_spatial_velocity,
    simplify_plant,
)
from drake_torque_control_study.systems import (
    declare_simple_cache,
    maybe_attach_zoh,
    simulator_initialize_repeatedly,
)
from drake_torque_control_study.trajectories import (
    make_se3_spline_trajectory,
)
from drake_torque_control_study.geometry import xyz_rpy_deg
from drake_torque_control_study.misc import (
    DirectPlant,
    EulerAccelPlant,
    SimpleLogger,
    load_pickle,
...
def make_osc_gains():
    return OscGains.via_damping_ratio(kp_t=300.0, kp_p=30.0)
...
def make_panda_limits(plant):
    ...
    # Narrow velocity limit.
    plant_limits.v = plant_limits.v.scaled(0.95)
    # Acceleration limits may be redundant / not commensurate with torque
    # limits. Remove for now.
